<?php $__env->startSection('title', 'Kelola Transaksi — Admin'); ?>
            ...
            <div class="flex flex-wrap items-center gap-3">
                <a href="<?php echo e(route('admin.dashboard')); ?>"
                   class="inline-flex items-center gap-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 px-4 py-2.5 text-xs font-bold text-slate-300 transition-all tracking-wide">
                    ...
                    <a href="<?php echo e(route('admin.orders.report.pdf')); ?>"
        ...
        <div class="rounded-3xl panel-card-glass overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-white/5 bg-white/5 font-medium text-slate-400 text-sm flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <div class="flex items-center gap-3">
                    <form method="GET" action="<?php echo e(route('admin.orders.index')); ?>" class="flex items-center gap-2">
                        <input name="q" value="<?php echo e($q ?? ''); ?>" placeholder="Cari invoice, buyer, seller..."
                        ...
                        <a href="<?php echo e(route('admin.orders.index')); ?>" class="rounded-xl border border-white/5 px-3 py-2 text-sm text-slate-300">Reset</a>
                    </form>
                    <span class="text-xs font-mono bg-black/20 text-slate-500 px-2 py-0.5 rounded border border-white/5 w-max">Total: <?php echo e($orders->total()); ?> Records</span>
                </div>
            </div>
                ...
                <div class="overflow-x-auto scroller-clean">
                    <table class="min-w-full divide-y divide-white/5 text-sm text-left text-slate-300">
                        <thead class="bg-black/30 text-[11px] font-bold uppercase tracking-wider text-slate-500 border-b border-white/5">
                                                        <tr>
                                                                <th class="px-6 py-4">
                                                                    <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'order_code', 'direction' => ($sort === 'order_code' && $direction === 'desc') ? 'asc' : 'desc'])); ?>">Nomor Invoice
                                                                        <?php if(($sort ?? '') === 'order_code'): ?><span class="text-xs"><?php echo e($direction === 'asc' ? '↑' : '↓'); ?></span><?php endif; ?>
                                                                    </a>
                                                                </th>
                                                                <th class="px-6 py-4">
                                                                    <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'created_at', 'direction' => ($sort === 'created_at' && $direction === 'desc') ? 'asc' : 'desc'])); ?>">Tanggal Transaksi
                                                                        <?php if(($sort ?? '') === 'created_at'): ?><span class="text-xs"><?php echo e($direction === 'asc' ? '↑' : '↓'); ?></span><?php endif; ?>
                                                                    </a>
                                                                </th>
                                                                <th class="px-6 py-4">Pihak Buyer</th>
                                                                <th class="px-6 py-4">Pihak Seller</th>
                                                                <th class="px-6 py-4">
                                                                    <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'grand_total', 'direction' => ($sort === 'grand_total' && $direction === 'desc') ? 'asc' : 'desc'])); ?>">Grand Total
                                                                        <?php if(($sort ?? '') === 'grand_total'): ?><span class="text-xs"><?php echo e($direction === 'asc' ? '↑' : '↓'); ?></span><?php endif; ?>
                                                                    </a>
                                                                </th>
                                ...
                                <tr class="hover:bg-white/5 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex flex-col">
                                            <span class="text-amber-400 invoice-token text-sm tracking-wide"><?php echo e($order->invoice_number); ?></span>
                                            <span class="text-[10px] text-slate-500 font-mono mt-0.5 uppercase">Code: <?php echo e($order->order_code); ?></span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2 max-w-[180px]">
                                            ...
                                            <div class="min-w-0">
                                                <span class="font-bold text-white tracking-tight break-words"><?php echo e($order->buyer?->name ?? 'Deleted User'); ?></span>
                                                <p class="mt-1 text-[11px] text-slate-500">
                                                    <?php echo e($order->created_at?->translatedFormat('d F Y H:i')); ?>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2 max-w-[180px]">
                                            ...
                                            <span class="font-bold text-slate-300 tracking-tight break-words"><?php echo e($order->seller?->name ?? 'Deleted Store'); ?></span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        ?>
                                        <span class="pill-status <?php echo e($statusClass); ?>">
                                    ...
                                    <td class="px-6 py-4 text-right whitespace-nowrap">
                                        <a href="<?php echo e(route('admin.orders.show', $order->order_code)); ?>" 
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\0_Project_VS_Code\3. Sistem Basis Data\TB-K1-Database\lapak_gaming\resources\views\admin\orders\index.blade.php ENDPATH**/ ?>