

<?php
    $displayImage = $product->image_url;
?>

<a href="<?php echo e(route('products.show', $product->slug)); ?>" class="product-card group relative flex flex-col">

  
  <?php if(($product->type ?? '') === 'topup'): ?>
    <span class="ribbon ribbon-blue">Top Up</span>
  <?php elseif($product->sold_count > 100): ?>
    <span class="ribbon">Bestseller</span>
  <?php endif; ?>

  
  <div class="relative aspect-square overflow-hidden" style="background:#090E1A;">
    
    <img src="<?php echo e($displayImage); ?>"
     alt="<?php echo e($product->name); ?>"
     class="w-full h-full object-cover transition-transform duration-400 group-hover:scale-108"
     loading="lazy"
     onerror="this.onerror=null;this.src='https://placehold.co/600x600/0f172a/38bdf8?text=Lapak+Gaming';">

    
    <?php if(($product->stock ?? 1) === 0): ?>
      <div class="absolute inset-0 flex items-center justify-center" style="background:rgba(6,10,18,0.7);">
        <span class="badge badge-red text-xs">Habis</span>
      </div>
    <?php elseif(($product->stock ?? 99) <= 5): ?>
      <div class="absolute bottom-2 right-2">
        <span class="badge badge-orange" style="font-size:0.6rem;">Stok <?php echo e($product->stock); ?></span>
      </div>
    <?php endif; ?>

    
    <div class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-end p-3">
      <span class="text-xs font-display font-semibold text-white bg-brand-600/90 px-2.5 py-1 rounded-lg">Lihat Produk →</span>
    </div>
  </div>

  
  <div class="flex flex-col flex-1 p-3">
    <p class="text-[10px] text-brand-400 font-display font-semibold uppercase tracking-wide mb-1">
      <?php echo e($product->category->name ?? 'Game'); ?>

    </p>
    <h3 class="text-sm font-semibold text-slate-100 line-clamp-2 leading-snug mb-2 flex-1">
      <?php echo e($product->name); ?>

    </h3>
    <div class="mt-auto">
      <p class="font-display font-bold text-base text-white"><?php echo e($product->formatted_price); ?></p>
      <div class="flex items-center justify-between mt-1.5">
        <div class="flex items-center gap-1">
          <svg class="w-3 h-3 text-amber-400" viewBox="0 0 20 20" fill="currentColor">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
          </svg>
          <span class="text-xs text-slate-400"><?php echo e(number_format($product->rating ?? 0, 1)); ?></span>
          <span class="text-xs text-slate-600">(<?php echo e($product->review_count ?? 0); ?>)</span>
        </div>
        <span class="text-[10px] text-slate-500">
          <?php echo e(number_format($product->sold_count ?? 0)); ?> terjual
        </span>
      </div>
    </div>
  </div>
</a><?php /**PATH D:\0_Project_VS_Code\3. Sistem Basis Data\TB-K1-Database\lapak_gaming\resources\views\components\product-card.blade.php ENDPATH**/ ?>