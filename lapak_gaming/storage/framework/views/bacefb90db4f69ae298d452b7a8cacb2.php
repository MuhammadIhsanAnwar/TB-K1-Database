...
<div class="min-h-screen py-12 px-4 relative overflow-hidden dashboard-transparent">
    ...
    <div class="mx-auto max-w-4xl rounded-3xl p-6 sm:p-8 form-card-glass relative z-10">
        ...
        <form action="<?php echo e(route('seller.produk.update', $produk)); ?>" method="POST" enctype="multipart/form-data" class="mt-6 space-y-5">
            <?php echo csrf_field(); ?>
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                <div class="block">
                    ...
                    <input name="name" type="text" value="<?php echo e(old('name', $produk->name)); ?>" class="mt-2 w-full rounded-xl input-glass px-4 py-3 text-sm text-white outline-none" required />
                </div>
                <div class="block">
                    <span class="text-xs font-bold uppercase tracking-wider text-slate-400">Harga (Rp)</span>
                    <input name="price" type="number" value="<?php echo e(old('price', $produk->price)); ?>" class="mt-2 w-full rounded-xl input-glass px-4 py-3 text-sm text-white outline-none" required />
                </div>
            </div>
                ...
                <div class="block">
                    ...
                    <select name="category_id" class="mt-2 w-full rounded-xl input-glass px-4 py-3 text-sm text-white outline-none" required>
                        ...
                        <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($category->id); ?>" <?php if(old('category_id', $produk->category_id) == $category->id): echo 'selected'; endif; ?>><?php echo e($category->name); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
                <div class="block">
                    ...
                    <select name="type" class="mt-2 w-full rounded-xl input-glass px-4 py-3 text-sm text-white outline-none" required>
                        <?php $__currentLoopData = ['topup' => '⚡ Topup Kilat', 'item' => '⚔️ Item & Skin', 'akun' => '👤 Akun Game', 'voucher' => '🎫 Voucher Digital', 'gamekey' => '🔑 Game Key']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($value); ?>" <?php if(old('type', $produk->type) === $value): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
            </div>
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                <div class="block">
                    ...
                    <input name="stock" type="number" value="<?php echo e(old('stock', $produk->stock)); ?>" class="mt-2 w-full rounded-xl input-glass px-4 py-3 text-sm text-white outline-none" required />
                </div>
                <div class="block">
                    ...
                    <select name="status" class="mt-2 w-full rounded-xl input-glass px-4 py-3 text-sm text-white outline-none" required>
                        <?php $__currentLoopData = ['published' => '⚡ Published (Tampilkan)', 'draft' => '💤 Draft (Sembunyikan)', 'archived' => '📁 Archived (Arsip)']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($value); ?>" <?php if(old('status', $produk->status) === $value): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
            </div>
                ...
                <?php if($produk->image_paths): ?>
                    <div class="mb-5">
                        ...
                        <div class="grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4">
                            <?php $__currentLoopData = $produk->image_paths; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $imagePath): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="relative rounded-xl overflow-hidden group border border-white/5 bg-black/40 backdrop-blur-sm p-1" data-image-index="<?php echo e($index); ?>">
                                    <img src="<?php echo e(Storage::disk('public_app_public')->url($imagePath)); ?>" 
                                         class="w-full h-24 object-cover rounded-lg" 
                                         alt="Produk <?php echo e($index + 1); ?>" />
                                    <button type="button" 
                                    ...
                                    <div class="absolute bottom-1 left-1 right-1 bg-black/60 px-2 py-1 rounded-b-lg">
                                        <p class="text-[9px] font-mono text-slate-400 truncate"><?php echo e(basename($imagePath)); ?></p>
                                    </div>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
            ...
            <div class="block border-t border-white/5 pt-4">
                ...
                <textarea name="description" rows="5" class="mt-2 w-full rounded-xl input-glass px-4 py-3 text-sm text-white outline-none resize-none"><?php echo e(old('description', $produk->description)); ?></textarea>
            </div>

            
            <div class="mt-6 border-t border-white/5 pt-5 flex items-center justify-end gap-3">
                <a href="<?php echo e(route('seller.produk.index')); ?>" class="rounded-xl bg-white/5 hover:bg-white/10 border border-white/5 px-5 py-3 text-xs font-bold text-slate-300 transition-colors tracking-wide">
                    BATALKAN PERUBAHAN
                </a>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\0_Project_VS_Code\3. Sistem Basis Data\TB-K1-Database\lapak_gaming\resources\views\seller\products\edit.blade.php ENDPATH**/ ?>