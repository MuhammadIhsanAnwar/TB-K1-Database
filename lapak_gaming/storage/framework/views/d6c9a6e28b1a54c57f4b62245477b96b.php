    ...
    <div class="mx-auto max-w-6xl space-y-8 relative z-10 px-4">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/40 pb-4">
            <div class="inline-flex rounded-xl p-1 glossy-tabs">
                <a href="<?php echo e(route('seller.produk.index', ['status' => 'active'])); ?>"
                   class="rounded-lg px-5 py-2 text-xs font-bold uppercase tracking-wider transition-all <?php echo e($status === 'active' ? 'bg-amber-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'); ?>">
                    📦 Produk Aktif
                </a>
                <a href="<?php echo e(route('seller.produk.index', ['status' => 'archived'])); ?>"
                   class="rounded-lg px-5 py-2 text-xs font-bold uppercase tracking-wider transition-all <?php echo e($status === 'archived' ? 'bg-amber-500 text-slate-950 shadow-md' : 'text-slate-400 hover:text-white'); ?>">
                    📁 Produk Arsip
                </a>
            </div>
            
            <div class="text-xs text-slate-500 font-medium bg-black/20 px-3 py-1.5 rounded-lg border border-white/5">
                Total Koleksi: <span class="text-slate-300 font-bold"><?php echo e($products->total()); ?> Item</span>
            </div>
        </div>
            ...
            <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="group rounded-3xl p-5 md:p-6 panel-card-glass">
                    <div class="flex flex-col gap-5 md:flex-row md:items-center">
                        ...
                        <div class="relative shrink-0 mx-auto md:mx-0">
                            <img src="<?php echo e($product->image_url); ?>"
                                 alt="Gambar <?php echo e($product->name); ?>"
                        ...
                        <div class="flex-1 text-center md:text-left min-w-0">
                            <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-3">
                                <h2 class="text-xl font-bold text-white tracking-tight truncate"><?php echo e($product->name); ?></h2>
                                <div class="flex items-center justify-center gap-2 mt-1 md:mt-0">
                                    ...
                                    <span class="px-2.5 py-0.5 text-[10px] font-bold rounded-md uppercase tracking-wider <?php echo e($product->status == 'published' ? 'status-published' : 'status-archived'); ?>">
                                        <?php echo e($product->status == 'published' ? '⚡ Published' : '💤 Archived'); ?>
                            ...
                            <p class="mt-2 text-sm text-slate-400 font-medium flex items-center justify-center md:justify-start gap-2">
                                ...
                                <span class="text-slate-300 font-semibold"><?php echo e($product->category?->name ?? 'Tanpa Kategori'); ?></span>
                            </p>
                        </div>
                        ...
                        <div class="text-center md:text-right border-y md:border-none border-white/5 py-3 md:py-0">
                            ...
                            <div class="text-2xl font-extrabold text-amber-400">
                                <span class="text-sm font-bold text-amber-500/70">Rp</span> <?php echo e(number_format($product->price, 0, ',', '.')); ?>
                    ...
                    <div class="mt-5 flex flex-col sm:flex-row justify-end items-center gap-2 border-t border-white/5 pt-4">
                        <a href="<?php echo e(route('seller.produk.edit', $product)); ?>" 
                        ...
                        <?php if($status === 'active'): ?>
                            <form action="<?php echo e(route('seller.produk.destroy', $product)); ?>" method="POST" class="w-full sm:w-auto">
                        ...
                        <?php else: ?>
                            <form action="<?php echo e(route('seller.produk.activate', $product)); ?>" method="POST" class="w-full sm:w-auto">
                                <?php echo csrf_field(); ?>
                                <button type="submit" 
                            ...
                            <form action="<?php echo e(route('seller.produk.forceDestroy', $product)); ?>" method="POST" class="w-full sm:w-auto" onsubmit="return confirm('Hapus produk ini permanen? Tindakan ini tidak bisa dibatalkan.');">
                                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                <button type="submit" 
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\0_Project_VS_Code\3. Sistem Basis Data\TB-K1-Database\lapak_gaming\resources\views\seller\products\index.blade.php ENDPATH**/ ?>