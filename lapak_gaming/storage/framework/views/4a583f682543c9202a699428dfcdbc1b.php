  ...
  <div class="dashboard-container px-6 lg:px-8 space-y-8 relative z-10">
    ...
    <div class="mt-4 mb-2 flex items-center justify-end gap-3">
      <form method="GET" action="<?php echo e(route('admin.users.index')); ?>" class="flex items-center gap-2">
        ...
        <input type="hidden" name="sort" value="<?php echo e($sort ?? 'created_at'); ?>" />
        <input type="hidden" name="direction" value="<?php echo e($direction ?? 'desc'); ?>" />
        <input name="q" value="<?php echo e($q ?? ''); ?>" placeholder="Cari nama atau email..."
        ...
        <a href="<?php echo e(route('admin.users.index')); ?>" class="rounded-xl border border-white/5 px-3 py-2 text-sm text-slate-300">Reset</a>
      </form>
    </div>
    ...
    <?php if($errors->any()): ?>
      <div class="flex items-start gap-3 rounded-2xl border border-rose-500/30 bg-rose-500/5 p-4 backdrop-blur-md">
        ...
        <p class="text-sm font-medium text-rose-300"><?php echo e($errors->first()); ?></p>
      </div>
    <?php endif; ?>
    ...
    <div class="overflow-x-auto table-wrapper">
      <div class="inline-flex gap-1 rounded-2xl p-1.5 glossy-tabs min-w-max">
        ...
        <a href="<?php echo e(route('admin.users.index', ['tab' => 'users'])); ?>"
           class="tab-btn <?php echo e($tab === 'users' ? 'active' : ''); ?>">
          User Reguler
          <span class="tab-badge"><?php echo e($regularUsers->total()); ?></span>
        </a>

        <a href="<?php echo e(route('admin.users.index', ['tab' => 'sellers'])); ?>"
           class="tab-btn <?php echo e($tab === 'sellers' ? 'active' : ''); ?>">
          Mitra Seller
          <span class="tab-badge"><?php echo e($sellers->total()); ?></span>
        </a>

        <a href="<?php echo e(route('admin.users.index', ['tab' => 'applications'])); ?>"
           class="tab-btn <?php echo e($tab === 'applications' ? 'active' : ''); ?>">
          Pengajuan Toko
        ...
        <a href="<?php echo e(route('admin.users.index', ['tab' => 'pending_verification'])); ?>"
           class="tab-btn <?php echo e($tab === 'pending_verification' ? 'active' : ''); ?>">
          Pending Verifikasi Email
          <?php if($pendingVerifications->total() > 0): ?>
            <span class="tab-badge badge-pending"><?php echo e($pendingVerifications->total()); ?></span>
          <?php endif; ?>
        </a>
          ...
          <div class="overflow-x-auto table-wrapper">
            <table class="min-w-full divide-y divide-white/5 text-sm text-left text-slate-300">
              <thead class="bg-black/30 text-[11px] font-bold uppercase tracking-wider text-slate-500 border-b border-white/5">
                <tr>
                  <th class="px-6 py-4">
                    <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'name', 'direction' => ($sort === 'name' && $direction === 'desc') ? 'asc' : 'desc', 'tab' => 'users'])); ?>">Nama / Pengguna
                      <?php if(($sort ?? '') === 'name'): ?>
                        <span class="text-xs"><?php echo e($direction === 'asc' ? '↑' : '↓'); ?></span>
                      <?php endif; ?>
                    </a>
                  </th>
                  <th class="px-6 py-4">
                    <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'email', 'direction' => ($sort === 'email' && $direction === 'desc') ? 'asc' : 'desc', 'tab' => 'users'])); ?>">Alamat Email
                      <?php if(($sort ?? '') === 'email'): ?>
                        <span class="text-xs"><?php echo e($direction === 'asc' ? '↑' : '↓'); ?></span>
                      <?php endif; ?>
                    </a>
                  ...
                  <th class="px-6 py-4">
                    <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'created_at', 'direction' => ($sort === 'created_at' && $direction === 'desc') ? 'asc' : 'desc', 'tab' => 'users'])); ?>">Tanggal Gabung
              ...
              <tbody class="divide-y divide-white/5">
                <?php $__currentLoopData = $regularUsers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <tr class="hover:bg-white/5 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center gap-3">
                        <img src="<?php echo e($user->avatar_url); ?>" alt="Avatar <?php echo e($user->name); ?>" class="w-9 h-9 rounded-full object-cover border border-white/10 bg-black/20" />
                        <div>
                          <p class="font-bold text-white"><?php echo e($user->name); ?></p>
                          <p class="text-xs text-slate-500 font-mono">UID #<?php echo e($user->id); ?></p>
                        </div>
                      </div>
                    </td>
                    <td class="px-6 py-4 text-slate-400 whitespace-nowrap font-medium"><?php echo e($user->email); ?></td>
                    <td class="px-6 py-4 text-slate-500 text-xs whitespace-nowrap font-medium"><?php echo e($user->created_at->format('d M Y')); ?></td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span class="pill <?php echo e($user->status === 'active' ? 'pill-active' : 'pill-suspended'); ?>">
                        <span class="w-1.5 h-1.5 rounded-full <?php echo e($user->status === 'active' ? 'bg-emerald-400' : 'bg-red-400'); ?>"></span>
                        <?php echo e($user->status === 'active' ? 'Aktif' : 'Suspended'); ?>

                      </span>
                      <?php if($user->status === 'suspended' && $user->suspend_reason): ?>
                        <p class="text-[11px] text-rose-400/80 mt-1 max-w-[220px] truncate font-medium" title="<?php echo e($user->suspend_reason); ?>">
                          Reason: <?php echo e($user->suspend_reason); ?>
                    ...
                    <td class="px-6 py-4 text-right whitespace-nowrap">
                      <?php if($user->status === 'active'): ?>
                        <button data-user-id="<?php echo e($user->id); ?>" data-user-name="<?php echo e(e($user->name)); ?>" onclick="openSuspendModal(this.dataset.userId, this.dataset.userName)"
                      ...
                      <?php else: ?>
                        <form method="POST" action="<?php echo e(route('admin.users.status', $user)); ?>" class="inline-block">
    ...
    <?php if($tab === 'pending_verification'): ?>
      <div class="rounded-3xl panel-card-glass overflow-hidden section-spacing">
        <div class="px-6 py-4 border-b border-white/5 bg-white/5 font-medium text-slate-300 text-sm">
        ...
        <?php else: ?>
          <div class="overflow-x-auto table-wrapper">
            <table class="min-w-full divide-y divide-white/5 text-sm text-left text-slate-300">
              ...
              <tbody class="divide-y divide-white/5">
                <?php $__currentLoopData = $pendingVerifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <tr class="hover:bg-white/5 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center gap-3">
                        <img src="<?php echo e($user->avatar_url); ?>" alt="Avatar <?php echo e($user->name); ?>" class="w-9 h-9 rounded-full object-cover border border-white/10 bg-black/20" />
                    ...
                    <td class="px-6 py-4 text-slate-400 whitespace-nowrap font-medium"><?php echo e($user->email); ?></td>
                    <td class="px-6 py-4 capitalize font-semibold text-xs text-slate-300 whitespace-nowrap">🕹️ <?php echo e($user->role); ?></td>
                    <td class="px-6 py-4 text-slate-500 text-xs whitespace-nowrap font-medium"><?php echo e($user->created_at->format('d M Y H:i')); ?></td>
                    <td class="px-6 py-4 whitespace-nowrap">
          ...
          <div class="px-6 py-4 border-t border-white/5 flex justify-center">
            <?php echo e($pendingVerifications->links()); ?>
          ...
          <div class="overflow-x-auto table-wrapper">
            <table class="min-w-full divide-y divide-white/5 text-sm text-left text-slate-300">
              <thead class="bg-black/30 text-[11px] font-bold uppercase tracking-wider text-slate-500 border-b border-white/5">
                <tr>
                  <th class="px-6 py-4">
                    <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'name', 'direction' => ($sort === 'name' && $direction === 'desc') ? 'asc' : 'desc', 'tab' => 'sellers'])); ?>">Pemilik Lapak
                      <?php if(($sort ?? '') === 'name'): ?>
                        <span class="text-xs"><?php echo e($direction === 'asc' ? '↑' : '↓'); ?></span>
                      <?php endif; ?>
                    </a>
                  ...
                  <th class="px-6 py-4">
                    <a href="<?php echo e(request()->fullUrlWithQuery(['sort' => 'email', 'direction' => ($sort === 'email' && $direction === 'desc') ? 'asc' : 'desc', 'tab' => 'sellers'])); ?>">Email Kontak
                      <?php if(($sort ?? '') === 'email'): ?>
                        <span class="text-xs"><?php echo e($direction === 'asc' ? '↑' : '↓'); ?></span>
                      <?php endif; ?>
                    </a>
              ...
              <tbody class="divide-y divide-white/5">
                <?php $__currentLoopData = $sellers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $seller): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                  <tr class="hover:bg-white/5 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center gap-3">
                        <img src="<?php echo e($seller->avatar_url); ?>" alt="Avatar <?php echo e($seller->name); ?>" class="w-9 h-9 rounded-full object-cover border border-white/10" />
                        <div>
                          <p class="font-bold text-white"><?php echo e($seller->name); ?></p>
                          <p class="text-xs text-slate-500 font-mono">UID #<?php echo e($seller->id); ?></p>
                        </div>
                      </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <div class="flex items-center gap-2.5">
                        <?php if($seller->shop_photo): ?>
                          <img src="<?php echo e($seller->shop_photo_url ?? asset('storage/' . $seller->shop_photo)); ?>" alt="Shop Thumbnail" class="shop-thumb" />
                        <?php endif; ?>
                        <span class="text-white font-bold text-sm tracking-tight"><?php echo e($seller->shop_name ?? $seller->name); ?></span>
                      </div>
                    </td>
                    <td class="px-6 py-4 text-slate-400 font-medium whitespace-nowrap"><?php echo e($seller->email); ?></td>
                    <td class="px-6 py-4 whitespace-nowrap">
                      <span class="pill <?php echo e($seller->status === 'active' ? 'pill-active' : 'pill-suspended'); ?>">
                        <span class="w-1.5 h-1.5 rounded-full <?php echo e($seller->status === 'active' ? 'bg-emerald-400' : 'bg-red-400'); ?>"></span>
                        <?php echo e($seller->status === 'active' ? 'Verified Lapak' : 'Suspended'); ?>

                      </span>
                      <?php if($seller->status === 'suspended' && $seller->suspend_reason): ?>
                        <p class="text-[11px] text-rose-400/80 mt-1 max-w-[200px] truncate font-medium" title="<?php echo e($seller->suspend_reason); ?>">
                          Reason: <?php echo e($seller->suspend_reason); ?>
                    ...
                    <td class="px-6 py-4 text-right whitespace-nowrap">
                      <?php if($seller->status === 'active'): ?>
                        <div class="inline-flex gap-2">
                                <button data-user-id="<?php echo e($seller->id); ?>" data-shop-name="<?php echo e(e($seller->shop_name ?? $seller->name)); ?>" onclick="openSuspendShopModal(this.dataset.userId, this.dataset.shopName)"
                                  class="rounded-xl bg-rose-500/10 border border-rose-500/20 px-4 py-2 text-xs font-bold text-rose-400 hover:bg-rose-600 hover:text-white transition-all">
                            Suspend Toko
                          </button>

                                <button data-user-id="<?php echo e($seller->id); ?>" data-user-name="<?php echo e(e($seller->name)); ?>" onclick="openSuspendModal(this.dataset.userId, this.dataset.userName)"
                      ...
                      <?php else: ?>
                        <form method="POST" action="<?php echo e(route('admin.users.status', $seller)); ?>" class="inline-block">
                          <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                          <input type="hidden" name="status" value="active" />
                          <button type="submit" class="rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 text-xs font-bold text-emerald-400 hover:bg-emerald-500 hover:text-slate-950 transition-all">
                            Aktifkan Kembali
        ...
        <?php else: ?>
          <div class="rounded-2xl border border-amber-500/20 bg-amber-500/5 px-5 py-3.5 backdrop-blur-md flex items-center gap-2.5">
            ...
            <p class="text-sm font-medium text-amber-300">
              Menunggu Tindakan: Terdapat <strong class="font-extrabold text-white"><?php echo e($applications->total()); ?></strong> formulir permohonan pendaftaran mitra baru.
            </p>
          </div>

          <?php $__currentLoopData = $applications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $applicant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="rounded-3xl panel-card-glass overflow-hidden group transition-all application-card">
              ...
              <div class="flex flex-col sm:flex-row sm:items-center gap-4 px-6 pt-6 pb-4">
                <img src="<?php echo e($applicant->avatar_url); ?>" alt="User <?php echo e($applicant->name); ?>" class="w-14 h-14 rounded-2xl object-cover border border-white/5 bg-black/30 shrink-0 shadow-inner" />
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <h2 class="text-lg font-bold text-white tracking-tight"><?php echo e($applicant->name); ?></h2>
                    <span class="pill pill-pending">Review Pending</span>
                  </div>
                  <p class="text-sm text-slate-400 font-medium"><?php echo e($applicant->email); ?></p>
                  <p class="text-xs text-slate-500 font-medium mt-0.5">Tanggal Pengajuan: <span class="text-slate-400 font-mono"><?php echo e($applicant->updated_at->format('d M Y, H:i')); ?></span></p>
                </div>
              </div>
              ...
              <div class="border-t border-white/5 px-6 py-5 grid gap-6 sm:grid-cols-2 bg-black/10">
                <div>
                  ...
                  <?php if($applicant->shop_photo): ?>
                    <img src="<?php echo e($applicant->shop_photo_url ?? asset('storage/' . $applicant->shop_photo)); ?>"
                         alt="Foto toko <?php echo e($applicant->shop_name); ?>"
                         ...
                         data-fallback-src="<?php echo e('https://ui-avatars.com/api/?name=' . urlencode($applicant->shop_name ?? 'Toko') . '&size=300&background=1e293b&color=94a3b8'); ?>"
                ...
                <div class="space-y-4">
                  <div>
                    ...
                    <p class="font-bold text-lg tracking-tight text-amber-400"><?php echo e($applicant->shop_name ?? '—'); ?></p>
                  </div>
                  <div>
                    <p class="text-[10px] uppercase tracking-wider font-bold text-slate-500 mb-1">Rencana Deskripsi & Komoditas Jualan</p>
                    <p class="text-slate-300 text-sm leading-relaxed bg-black/20 p-3 rounded-xl border border-white/5"><?php echo e($applicant->shop_description ?? 'Tidak menyertakan deskripsi.'); ?></p>
                  </div>
                </div>
              </div>
              ...
              <div class="border-t border-white/5 px-6 py-4 flex flex-col sm:flex-row justify-end items-center gap-2.5">
                <form method="POST" action="<?php echo e(route('admin.users.approve-seller', $applicant)); ?>" class="w-full sm:w-auto">
                  <?php echo csrf_field(); ?>
                  <button type="submit"
                          onclick="return confirm('Approve hak jualan dan buka lapak pedagang untuk <?php echo e(addslashes($applicant->name)); ?>?')"
                ...
                <button data-user-id="<?php echo e($applicant->id); ?>" data-user-name="<?php echo e(e($applicant->name)); ?>" onclick="openRejectModal(this.dataset.userId, this.dataset.userName)"
          ...
          <div class="mt-4 flex justify-center"><?php echo e($applications->links()); ?></div>
        <?php endif; ?>
      </div>
    <?php endif; ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\0_Project_VS_Code\3. Sistem Basis Data\TB-K1-Database\lapak_gaming\resources\views\admin\users\index.blade.php ENDPATH**/ ?>