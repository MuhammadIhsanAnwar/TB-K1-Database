...
<div class="min-h-screen bg-surface py-12 px-4">
    <div class="max-w-4xl mx-auto">
        <div class="flex flex-col gap-4 mb-8 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h1 class="text-3xl font-bold surface-text">Notifikasi</h1>
                <p class="mt-2 text-sm surface-muted">Pilih kategori untuk melihat pesan yang paling relevan.</p>
            </div>
            <div class="flex items-center gap-4">
                ...
                <form action="{{ route('notifications.destroy-all') }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus semua notifikasi?')">
                    @csrf
                    <button type="submit" class="text-sm font-semibold text-accent hover:text-accent/80 transition-colors">
                        Hapus Semua
                    </button>
                </form>
            </div>
        ...
        <div class="mb-6 overflow-x-auto">
            <div class="inline-flex min-w-max gap-2 rounded-2xl border border-white/10 surface-panel p-1.5">
                @foreach($tabs as $key => $label)
                    <a href="{{ route('notifications.index', ['filter' => $key]) }}"
                       class="rounded-xl px-4 py-2 text-sm font-semibold transition {{ $filter === $key ? 'bg-primary text-white' : 'surface-muted hover:bg-surface-weak hover:surface-text' }}">
                        {{ $label }}
        ...
        @if($notifications->count() > 0)
            <div class="space-y-3">
                ...
                <div class="surface-panel rounded-lg p-4 border border-white/10 hover:border-white/20 transition-colors {{ !$notification->is_read ? 'border-purple-600/30' : '' }}">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-2">
                                ...
                                <span class="rounded-full border border-white/10 bg-surface-weak px-2 py-0.5 text-[11px] font-semibold surface-muted">
                                    {{ $notification->category_label }}
                                </span>
                            </div>
                            <p class="surface-muted text-sm">{{ $notification->body }}</p>
                            <p class="surface-muted text-xs mt-2">{{ $notification->created_at->diffForHumans() }}</p>
                        </div>
                        <div class="ml-4 flex flex-col items-end gap-2 shrink-0">
                            ...
                            @if(!$notification->is_read)
                                <form action="{{ route('notifications.read', $notification) }}" method="POST" class="inline">
                                    @csrf
                                    <button class="surface-muted hover:surface-text transition-colors text-sm font-semibold whitespace-nowrap">
                                        Tandai dibaca
                                    </button>
                                </form>
                            @endif
                            <form action="{{ route('notifications.destroy', $notification) }}" method="POST" class="inline" onsubmit="return confirm('Hapus notifikasi ini?')">
                                @csrf
                                @method('DELETE')
                                <button class="text-accent hover:text-accent/80 transition-colors text-sm font-semibold whitespace-nowrap">
                                    Hapus
                                </button>
                            </form>
                        </div>
        ...
        @else
        <div class="surface-panel rounded-xl p-12 text-center border border-white/10">
            ...
            <p class="surface-muted text-lg mb-2">Belum ada notifikasi</p>
            <p class="surface-muted text-sm">Notifikasi kategori ini akan muncul di sini.</p>
        </div>
        @endif
    </div>
</div>
