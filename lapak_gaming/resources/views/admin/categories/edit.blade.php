...
<div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-slate-200 md:ml-2">Edit</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-black tracking-tight text-white">Edit Kategori</h1>
        <p class="mt-1 text-sm text-slate-400">Perbarui data kategori "{{ $category->name }}".</p>
    </div>
        ...
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" class="input w-full" placeholder="Cth: Voucher Game" required>
                </div>
                ...
                <div>
                    ...
                    <input type="url" name="icon" id="icon" value="{{ old('icon', $category->icon) }}" class="input w-full font-mono text-sm" placeholder="Cth: https://example.com/icon.png">
                    <p class="mt-1.5 text-xs text-slate-500">Masukkan link URL untuk gambar ikon kategori (opsional).</p>
                </div>

                {{-- Gambar --}}
                <div>
                    <label class="mb-2 block text-sm font-bold text-slate-300">Gambar Saat Ini</label>
                    @if($category->image)
                        <div class="mb-4">
                            <img src="{{ asset('storage/' . $category->image) }}" alt="Current Image" class="h-20 rounded-lg border border-slate-700 object-cover">
                        </div>
                    @else
                        <p class="mb-4 text-sm text-slate-500 italic">Belum ada gambar.</p>
                    @endif
                    <label for="image" class="mb-2 block text-sm font-bold text-slate-300">Unggah Gambar Baru</label>
                ...
                <div class="flex items-center gap-3">
                    <label class="relative inline-flex cursor-pointer items-center">
                        <input type="checkbox" name="is_active" value="1" class="peer sr-only" @checked(old('is_active', $category->is_active))>
