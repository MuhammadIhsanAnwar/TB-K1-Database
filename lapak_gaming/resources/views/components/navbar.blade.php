...
<style>
  ...
  /* CATEGORY PILL TAGS (Clean, no weird bottom borders) */
  .cat-pill {
      display: inline-flex;
      align-items: center;
      gap: 0.4rem;
      ...
      <ul class="space-y-1">
        ...
        <li>
          <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="w-full flex items-center py-2.5 px-3 rounded-lg hover:bg-red-500/10 text-red-400 text-sm text-left">Keluar</button>
          </form>
        </li>
      </ul>
    </div>
...
{{-- ═══ DESKTOP NAVBAR ═══ --}}
<header id="main-navbar" class="fixed top-0 w-full z-40 transition-all duration-300 {{ ($isAdminRoute || $isAdminSettingsRoute) ? 'admin-navbar' : '' }}">
  ...
  {{-- SUB-BAR: CATEGORY TAGS (The Clean Version without weird lines) --}}
  @if(! $isAdminRoute)
...
<script>
  ...
  // Klik di luar dropdown akan menutup dropdownnya
  ...
  // Efek transisi kaca mengecil/menggelap pas di-scroll ke bawah
