...
<a href="{{ route('products.show', $product) }}" class="product-card group flex flex-col rounded-xl overflow-hidden transition-all duration-300 relative h-full">
  ...
  @if(($product->category?->slug ?? '') === 'top-up-game')
    <div class="absolute top-2 left-2 z-10 ribbon ribbon-accent">Top Up</div>
  @elseif(str_contains(strtolower($product->category?->name ?? ''), 'key'))
    <div class="absolute top-2 left-2 z-10 ribbon ribbon-blue">Game Key</div>
  @endif
  ...
  <div class="flex flex-col flex-1 p-3">
    ...
    <div class="mt-auto">
      ...
      <div class="flex items-center text-xs surface-muted gap-2">
        <div class="flex items-center gap-1">
          <svg class="w-3.5 h-3.5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
          </svg>
          <span class="font-medium surface-text">{{ number_format($rating, 1) }}</span>
        </div>
        <span class="w-1 h-1 rounded-full bg-gray-300"></span>
        <span class="surface-muted">{{ number_format($product->sold_count ?? 0) }} terjual</span>
      </div>
    </div>
  </div>
</a>