    ...
    <div class="mx-auto max-w-6xl space-y-8 relative z-10">
        ...
        @if($cartItems->isEmpty())
            ...
            <div class="rounded-[2rem] border border-slate-800 bg-slate-900/40 p-16 flex flex-col items-center justify-center text-center shadow-xl">
                <div class="w-32 h-32 bg-slate-950 rounded-full flex items-center justify-center mb-8 shadow-inner border border-slate-800 relative">
                ...
                <p class="text-slate-400 mb-8 max-w-sm mx-auto">Lapak Gaming punya ribuan voucher dan akun menanti untuk kamu amankan.</p>
            ...
            <div class="grid gap-8 lg:grid-cols-3 items-start">
                ...
                <div class="lg:col-span-2 space-y-4">
                    <div class="flex items-center justify-between px-2 mb-2">
                        ...
                        <span class="text-xs font-black text-slate-500 uppercase tracking-[0.2em] hidden sm:block">Total Harga</span>
                    </div>
                        ...
                        <div class="group rounded-2xl border border-slate-800 bg-slate-900/60 p-5 flex flex-col sm:flex-row gap-5 transition-all hover:border-amber-500/40 hover:bg-slate-900 shadow-md">
                            ...
                            <div class="flex-1 flex flex-col justify-center min-w-0">
                                <div class="flex items-center gap-2 mb-1.5">
                                    <span class="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[10px] font-black uppercase rounded">STOK TERSEDIA</span>
                                    <span class="px-2 py-0.5 bg-slate-800 text-slate-400 text-[10px] font-black uppercase rounded">{{ $item->product->type ?? 'ITEM' }}</span>
                                </div>
                                <h3 class="text-lg font-bold text-white leading-tight mb-1 truncate group-hover:text-amber-500 transition-colors">{{ $item->product->name }}</h3>
                            ...
                            {{-- Harga & Aksi --}}
                            <div class="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-4 sm:border-l border-slate-800 sm:pl-8">
                                ...
                                <form action="{{ route('cart.remove', $item->id) }}" method="POST">
                                    @csrf @method('DELETE')
                                    <button class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold text-slate-600 hover:text-rose-500 hover:bg-rose-500/10 transition-all" onclick="return confirm('Hapus item ini?')">
                ...
                <div class="lg:col-span-1 space-y-6 sticky top-6">
                    ...
                    <div class="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-5 shadow-lg">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-slate-400 leading-relaxed font-medium">Dana kamu aman. Saldo hanya diteruskan ke seller setelah pesanan berhasil kamu terima.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-[2rem] border border-slate-800 bg-slate-900/80 p-8 shadow-xl">
                        <h2 class="text-base font-black text-white mb-6 uppercase tracking-[0.2em] italic">Ringkasan Belanja</h2>
                        ...
                        <div class="space-y-4 border-b border-slate-800 pb-6 text-sm">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-white font-black text-base">Rp {{ number_format($cartItems->sum(fn($i) => $i->product->price), 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between items-center">
                                <span class="text-slate-500 font-bold uppercase tracking-tighter">Biaya Admin</span>
                                <span class="text-emerald-500 font-black bg-emerald-500/10 px-2 py-0.5 rounded text-[10px]">GRATIS</span>
                            </div>
                        </div>

                        <div class="py-6">
                            <span class="text-[10px] font-black text-slate-600 uppercase tracking-widest block mb-1">Total Tagihan</span>
                        ...
                        <form action="{{ route('checkout.store') }}" method="POST">
                            @csrf
                            <button class="w-full py-4 bg-amber-500 text-slate-950 font-black text-lg rounded-2xl hover:bg-amber-400 transition-all shadow-lg shadow-amber-500/20 active:scale-95 flex items-center justify-center gap-2">
                                BELI SEKARANG
                            </button>
                        </form>

                        <div class="mt-8 pt-6 border-t border-slate-800">
                            <div class="flex items-center justify-center gap-2 opacity-30 grayscale hover:grayscale-0 transition-all">
                                <img src="https://upload.wikimedia.org/wikipedia/commons/b/b5/PayPal.svg" class="h-3" alt="Paypal">
                                <img src="https://upload.wikimedia.org/wikipedia/commons/5/5e/Visa_Inc._logo.svg" class="h-2.5" alt="Visa">
                                <img src="https://upload.wikimedia.org/wikipedia/commons/2/2a/Mastercard-logo.svg" class="h-4" alt="Mastercard">
                            </div>
                            <p class="mt-4 text-[9px] text-slate-600 text-center font-bold tracking-widest leading-relaxed uppercase">
