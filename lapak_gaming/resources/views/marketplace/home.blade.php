...
@push('styles')
  <style>
    ...
    .cat-item:hover .cat-icon-box::after {
      opacity: 1;
    }
...
@section('content')
  <div class="home-page">
    ...
    <section class="hero-section">
      <div class="w-full mx-auto flex flex-col lg:flex-row items-center justify-between relative z-10"
        style="max-width: 1440px; padding-left: clamp(24px, 5vw, 80px); padding-right: clamp(24px, 5vw, 80px);">
        ...
        <div class="hidden lg:flex flex-1 w-full justify-center lg:justify-end mt-12 lg:mt-0"
          style="padding-left: clamp(0px, 4vw, 48px);">
              ...
              <div class="banner-marquee-track py-2">
                @foreach($heroLoopBanners as $banner)
                  <a href="{{ $banner->link_url ?: '#' }}"
                    ...
                    <img src="{{ $banner->image_url }}" alt="{{ $banner->title ?? 'Banner Promo' }}"
                    ...
                    <div class="absolute inset-x-0 bottom-0 p-5">
                      <span
                        class="inline-flex rounded-full bg-amber-500/15 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.22em] text-amber-300 mb-3">Hot
                        Games</span>
                      <h3 class="text-lg font-extrabold text-white line-clamp-2 leading-tight">
                        {{ $banner->title ?? 'Promo Spesial' }}</h3>
                      @if(!empty($banner->subtitle))
                        <p class="mt-1.5 text-sm text-slate-300 line-clamp-2">{{ $banner->subtitle }}</p>
                      @endif
                      <div
                        class="mt-4 inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/8 backdrop-blur-sm px-4 py-2 text-xs font-bold text-white">
                        Explore
              ...
              @foreach($heroBanners as $banner)
                <a href="{{ $banner->link_url ?: '#' }}" class="banner-card group relative aspect-[4/5]">
                  ...
                  <div class="absolute inset-x-0 bottom-0 p-4">
                    <span
                      class="inline-flex rounded-full bg-amber-500/15 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-[0.2em] text-amber-300 mb-2">Hot
                      Games</span>
                    ...
                    @endif
                    <div
                      class="mt-3 inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/8 px-3 py-1.5 text-[10px] font-bold text-white">
                      Explore
        ...
        <div class="cta-panel">
          <div style="position:relative; z-index:2;">
            ...
            <p class="text-slate-400 text-center max-w-lg mx-auto" style="margin-bottom:2.25rem; line-height:1.7;">
              Daftar gratis dan dapatkan akses ke ribuan produk digital dari seller terpercaya Indonesia. Transaksi aman dan
              pengiriman instan.
            </p>
    ...
    <section class="reveal-item home-wrap sec-gap-sm pb-16" id="faq">
      <div class="cat-panel" style="padding: 3.5rem 2rem;">
        ...
        {{-- Header FAQ --}}
        <div class="sec-header flex-col items-center text-center mx-auto max-w-2xl"
          style="align-items: center; justify-content: center; margin-bottom: 3.5rem;">
          ...
          <p class="sec-sub mt-2 max-w-lg mx-auto text-center" style="line-height: 1.6;">
            Temukan jawaban atas pertanyaan yang paling sering ditanyakan mengenai top up game, pembayaran, dan proses
            transaksi.
          </p>
        </div>
        ...
        <div class="faq-container max-w-3xl mx-auto space-y-3.5 relative z-10">
          ...
          @foreach($faqs as $index => $faq)
            <div
              class="faq-item group relative overflow-hidden rounded-[20px] border border-white/[0.05] bg-white/[0.015] transition-all duration-300 hover:bg-white/[0.03] hover:border-sky-500/30">
              {{-- Left Accent Line --}}
              <div
                class="faq-accent absolute left-0 top-0 bottom-0 w-[3px] bg-gradient-to-b from-sky-400 to-blue-600 opacity-0 transition-all duration-500 scale-y-0 origin-top">
              </div>

              <button
                class="faq-btn flex w-full items-center justify-between gap-4 p-5 sm:p-6 text-left cursor-pointer outline-none focus:outline-none">
                <div class="flex items-center gap-4 sm:gap-6">
                  <span
                    class="faq-num font-['Oxanium'] text-xl sm:text-2xl font-black text-white/10 transition-colors duration-300">{{ sprintf('%02d', $index + 1) }}</span>
                  <h3
                    class="faq-title font-semibold text-white/80 text-sm sm:text-base transition-colors duration-300 group-hover:text-sky-300">
                    {{ $faq['q'] }}</h3>
                </div>
                <div
                  class="faq-icon-box flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/5 transition-all duration-300 group-hover:border-sky-400/30 group-hover:bg-sky-500/10">
                  <svg class="faq-icon h-4 w-4 text-sky-400 transition-all duration-300" fill="none" viewBox="0 0 24 24"
