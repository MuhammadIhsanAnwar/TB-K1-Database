...
<section class="max-w-7xl mx-auto px-4 py-8">
  ...
  <div class="flex flex-col lg:flex-row gap-8">
    ...
    <aside class="w-full lg:w-64 shrink-0">
      <div class="surface-panel rounded-xl shadow-sm border border-white/10 p-5 sticky top-24">
        <h3 class="font-bold surface-text mb-4 flex items-center gap-2">
        ...
        <form action="{{ route('products.search') }}" method="GET" id="filter-form">
          ...
          {{-- Type Filter --}}
          <div class="mb-6">
            <h4 class="text-xs font-semibold surface-muted uppercase mb-3">Tipe Produk</h4>
            <div class="space-y-2">
          ...
          @if(request('type') || request('min_price') || request('max_price'))
            <a href="{{ route('products.search', ['q' => request('q'), 'category' => request('category')]) }}" class="block w-full text-center py-2 text-red-400 hover:brightness-110 text-sm font-semibold rounded transition-colors">Hapus Filter</a>
          @endif
        </form>
      </div>
    </aside>
    ...
    <div class="flex-1">
      ...
      {{-- Header --}}
      <div class="surface-panel rounded-xl p-6 mb-6 shadow-sm border border-white/10">
