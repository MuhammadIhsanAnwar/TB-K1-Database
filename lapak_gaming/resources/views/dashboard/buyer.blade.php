...
@section('content')
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="rounded-[2rem] border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="mt-2 text-3xl font-black">Rp {{ number_format($wallet->balance, 0, ',', '.') }}</div>
            <div class="mt-1 text-sm text-slate-500">Available: Rp {{ number_format($wallet->available_balance, 0, ',', '.') }}</div>
        </div>
        <div class="rounded-[2rem] border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
                ...
                @forelse ($orders as $order)
                    <div class="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950/40">
                        <div class="flex items-center justify-between text-sm">
                            ...
                            <span class="rounded-full bg-slate-200 px-3 py-1 text-xs font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-200">{{ $order->status }}</span>
                        </div>
