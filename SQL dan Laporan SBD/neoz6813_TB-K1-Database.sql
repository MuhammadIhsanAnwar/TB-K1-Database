...
--
-- Host: localhost:3306
-- Generation Time: May 25, 2026 at 08:10 AM
...
--
DELIMITER $$
CREATE TRIGGER `reduce_stock` AFTER INSERT ON `order_items` FOR EACH ROW UPDATE products
...
ALTER TABLE `marketplace_notifications`
  ...
  ADD KEY `marketplace_notifications_broadcast_id_foreign` (`broadcast_id`);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`neoz6813_tbk1database`@`localhost` SQL SECURITY DEFINER VIEW `view_penjualan`  AS SELECT `orders`.`id` AS `id`, `users`.`name` AS `name`, `orders`.`grand_total` AS `grand_total` FROM (`orders` join `users` on(`orders`.`buyer_id` = `users`.`id`)) ;
...
--
ALTER TABLE `marketplace_notifications`
  ADD CONSTRAINT `marketplace_notifications_broadcast_id_foreign` FOREIGN KEY (`broadcast_id`) REFERENCES `notification_broadcasts` (`id`) ON DELETE SET NULL,
